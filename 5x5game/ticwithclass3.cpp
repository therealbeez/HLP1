...
//gamestate is -1 for active game,0 for draw, and 1 for end
int gamestate = -1;
int player = 1;
int mul;
int imul;
...
//function to check wether user input is valid and error handling
char checklegal(){
...
int getmultiple(int x){
    //divides index by 5 and gets the multiple to determine row number
        mul = x/5;
        imul = static_cast<int>(mul);
        return imul;
    }

int checkwin(){
    //get row numbers of input
    int r= getmultiple(choiceindex);
    int rneg1= getmultiple(choiceindex-1);
    int rneg2= getmultiple(choiceindex-2);
    int rneg4= getmultiple(choiceindex-4);
    int rneg8= getmultiple(choiceindex-8);

    int rpos1= getmultiple(choiceindex+1);
    int rpos2= getmultiple(choiceindex+2);
    int rpos4= getmultiple(choiceindex+4);
    int rpos8= getmultiple(choiceindex+8);

    //checks for accross match
    if((squares[choiceindex] == mark)&&(squares[choiceindex - 1] == mark)&&(squares[choiceindex - 2] == mark)){
        //checks if inputs are on the same row
        if((r==rneg1)&&(r==rneg2)) return 1;
    }
    else if((squares[choiceindex] == mark)&&(squares[choiceindex - 1] == mark)&&(squares[choiceindex + 1] == mark))
        if((r==rneg1)&&(r==rpos1)) return 1;
    else if((squares[choiceindex] == mark)&&(squares[choiceindex + 1] == mark)&&(squares[choiceindex + 2] == mark))
        if((r==rpos1)&&(r==rpos2)) return 1;
    ...
    if ((squares[22] == mark)&&(squares[18] == mark)&&(squares[14] == mark))return 1;
    else if ((squares[22] == mark)&&(squares[18] == mark))return -1;
    else if((squares[choiceindex] == mark)&&(squares[choiceindex - 4] == mark)&&(squares[choiceindex - 8] == mark)){
        if((r!=rneg4)&&(r!=rneg8)) return 1;
        else return -1;
        
    }
    else if((squares[choiceindex] == mark)&&(squares[choiceindex - 4] == mark)&&(squares[choiceindex + 4] == mark)){
        if((r!=rneg4)&&(r!=rpos4)) return 1;
        else return -1;
    }
    else if((squares[choiceindex] == mark)&&(squares[choiceindex + 4] == mark)&&(squares[choiceindex + 8] == mark)){
        if((r!=rpos4)&&(r!=rpos8)) return 1;
        else return -1;
    }
    else{
        return -1;
